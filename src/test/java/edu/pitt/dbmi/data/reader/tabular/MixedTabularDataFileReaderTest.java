...
public class MixedTabularDataFileReaderTest {
     ...
     */
    @Test
    public void testReadInDataWithExcludeDiscreteColumns() throws IOException {
        Set<String> excludedColumns = new HashSet<>(Arrays.asList("X2", "X4", "X5", "X7", "X10"));
        for (Path dataFile : dataFiles) {
            MixedTabularDataReader dataReader = new MixedTabularDataFileReader(numberOfDiscreteCategories, dataFile, delimiter);
            dataReader.setCommentMarker(commentMarker);
            dataReader.setQuoteCharacter(quoteCharacter);
    ...
    public void testReadInDataWithExcludedColumns() throws IOException {
        Set<String> excludedColumns = new HashSet<>(Arrays.asList("X1", "X4", "X8", "X3", "X10", "X6"));
        for (Path dataFile : dataFiles) {
            MixedTabularDataReader dataReader = new MixedTabularDataFileReader(numberOfDiscreteCategories, dataFile, delimiter);
            dataReader.setCommentMarker(commentMarker);
            dataReader.setQuoteCharacter(quoteCharacter);
